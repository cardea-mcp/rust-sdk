...
use rmcp::{
    ...
    transport::streamable_http_client::StreamableHttpClientAuto,
};
...
async fn main() -> Result<()> {
    ...
    let client = StreamableHttpClientAuto::new(
        "client",
        server_did,
    ...
    tracing::info!("Resolved server endpoint: {}", client.url);

    let client_info = ClientInfo {
        protocol_version: Default::default(),
        capabilities: ClientCapabilities::default(),
        client_info: Implementation {
            name: "test streamable http tmcp client".to_string(),
